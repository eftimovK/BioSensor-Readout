...
static CPU_INT08U   phdc_dev_nbr;
static CPU_INT08U   phdc_cfg_nbr;
...
CPU_BOOLEAN  App_USBD_Init ()
{
    ...
    ok = App_USBD_CreateDevice(&USBD_DevCfg_ADI_Vendor,  &phdc_dev_nbr,  &phdc_cfg_nbr);
    if (ok != DEF_OK) {
        return (DEF_FAIL);
    }
    ok = App_USBD_PHDC_Init(phdc_dev_nbr, USBD_CFG_NBR_NONE, phdc_cfg_nbr);
    if (ok != DEF_OK) {
        return (DEF_FAIL);
    }

    ok = App_USBD_DevStart(phdc_dev_nbr);
    if (ok != DEF_OK) {
        return (DEF_FAIL);
    }

...
extern void Create_RAMDiskPrepareTask( CPU_INT08U msc_dev_nbr );
...
static CPU_INT08U   msc_dev_nbr;
static CPU_INT08U   msc_cfg_nbr;

extern USBD_DEV_CFG  USBD_DevCfg_ADI_PHDC;
extern USBD_DEV_CFG  USBD_DevCfg_ADI_MSC;
...
CPU_BOOLEAN  App_USBD_Init ()
{
    ...
    ok = App_USBD_CreateDevice(&USBD_DevCfg_ADI_MSC,  &msc_dev_nbr,  &msc_cfg_nbr);
    if (ok != DEF_OK) {
        return (DEF_FAIL);
    }
    ok = App_USBD_MSC_Init(msc_dev_nbr, USBD_CFG_NBR_NONE, msc_cfg_nbr);
    if (ok != DEF_OK) {
        return (DEF_FAIL);
    }

    ok = App_USBD_DevStart(msc_dev_nbr);
    if (ok != DEF_OK) {
        return (DEF_FAIL);
    }

...
/*****************************************************************************
 * @file:    RtcAlarmTest.c
 * @brief:   Real-Time Clock Alarm Test for ADuCM350
...
void rtc_Init (void) {
    ...
    rtc_ReportTime();
#else
    PERF("Use \"ADI_RTC_RESET\" preprocessor macro to recompute/reset RTC time.");
#endif
...
#else
    PERF("Use \"ADI_RTC_CALIBRATE\" preprocessor macro for RTC calibration.");
#endif
}
...
void rtc_AlarmTest (void) {
    ...
    /* program wake-up timerB (in seconds) as a stop-gap to interrupt from hibernation if the RTC ALARM fails */
    {
        ...
        if (adi_WUT_SetComparator(hWUT, ADI_WUT_COMPB, (2*RTC_ALARM_OFFSET*ADI_RTC_NUM_ALARMS) + adi_WUT_GetCurrentCount(hWUT)))
    ...
    if (!bRtcAlarmFlag)  FAIL("rtc_AlarmTest failed to get expected RTC ALARM interrupts");
